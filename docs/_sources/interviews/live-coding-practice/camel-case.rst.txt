...
The string follows camel case convention. This means the first word is
...
Our goal is to determine how many words are in the string, then print
that number out.
...
I want to hold my phrase in a list of the individual words. First, I
will create an empty list.

I will then need to create an empty string that will serve as a
temporary container.

I should be able to loop through the string by each individual
...
be the new uppercase character.

Finally I should be able to return the length of the list. This will
represent the number of words in the original string.
...
We can see in this string the first four letters are all lower case
(“t”, “h”, “i”, “s”), they will satisfy the if condition, and will be
added to temp_word.

On the fifth letter (“I”) the if statement is not true, so the else
statement is executed. This adds the temp_word (“this”) to the_words. It
then overwrites temp_word to be just the current letter (“I”).
...
The next letter is uppercase (“A”), thus temp_word is added to
the_words, and temp_word is set to the current letter (“A”).
...
The next letter is uppercase (“E”), thus temp_word is added to
the_words, and temp_word is set to the current letter (“E”).
...
Oops! I made a small mistake. I forgot to add the last word to
...
This will change our list the_words and allow the length function to
...
Another Successful Live Coding session! We employed all the steps and
...
We aren’t asked to do anything with the words that we have stored in our
...
Many of you may have done this on your attempt with this problem. That’s
